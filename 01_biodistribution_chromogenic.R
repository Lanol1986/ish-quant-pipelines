## =============================================================================
## PIPELINE 01 -- COMPOUND BIODISTRIBUTION, CHROMOGENIC ISH
##
## Probe directed against the therapeutic compound itself (an siRNA passenger or
## guide strand, or an ASO), chromogenic detection, brightfield, whole slide.
##
## Endpoint: DISTRIBUTION. Not knockdown, and not concentration.
##
...
##
## Usage
##   Rscript pipelines/01_biodistribution_chromogenic.R config/my_study.yml
##
## or from a notebook with the R kernel:
##   CONFIG_PATH <- "config/my_study.yml"
##   source("pipelines/01_biodistribution_chromogenic.R")
##
## Contents
...
main <- function(config_path) {
  ...
  message("\n== Pipeline 01: compound biodistribution ==\n", cfg$study$name, "\n")
  ...
  ## --- 2. Read ---------------------------------------------------------------
  probe <- cfg$assay$probes[[1]]$export_name %||% cfg$assay$probes[[1]]$name
  ...
  ## --- 3. QC before any result -----------------------------------------------
  qc$expression_regime <- qc_expression_regime(
    cell_data, count_col, countable_max = cfg$readout$countable_regime_max)
  print(qc$expression_regime |>
          dplyr::select(dplyr::any_of(c("group", "region", "animal_id",
                                        "pct_positive", "pct_pos_above_max",
                                        "regime"))))
  ...
  ## Control material for this assay is a control animal, which controls probe
  ## background on this tissue, and where available a negative-control probe on
  ## treated tissue, which additionally controls cross-reactivity. Which was
  ## used is recorded in the config either way.
  ...
  message("Threshold derived from control tissue: ",
          signif(thr_info$threshold, 4),
          "   (config value in use: ", cfg$readout$positivity_threshold, ")")

  threshold          <- cfg$readout$positivity_threshold
  qc$threshold_sweep <- qc_threshold_sweep(cell_data, count_col)

  ## --- 4. Animal-level summary, the analysis unit ----------------------------
  ...
  ## --- 5. Comparison against control -----------------------------------------
  ## In a biodistribution assay the control is usually at or near zero, and a
  ## fold change against it is not a usable quantity: its value is set by
  ## however many control cells happened to trip the threshold.
  ## relative_to_control() returns NA in that case, and the difference with an
  ## exact binomial interval is computed here instead.
  rel  <- relative_to_control(animal_tbl, "pct_positive", cfg, within = "region")
  null <- control_null_distribution(animal_tbl, "pct_positive", cfg,
                                    within = "region")
  ...
  positivity <- animal_tbl |>
    dplyr::group_by(region, group) |>
    dplyr::summarise(n_animals    = dplyr::n(),
                     mean_pct_pos = mean(pct_positive),
                     sd_pct_pos   = stats::sd(pct_positive),
                     total_pos    = sum(n_positive),
                     total_cells  = sum(n_cells),
                     .groups = "drop") |>
    dplyr::rowwise() |>
    dplyr::mutate(
      .ci            = list(stats::binom.test(total_pos, total_cells)$conf.int * 100),
      pooled_pct     = 100 * total_pos / total_cells,
      pooled_ci_low  = .ci[1],
      pooled_ci_high = .ci[2]) |>
    dplyr::select(-.ci) |>
    dplyr::ungroup()

  readr::write_csv(positivity, file.path(dirs$tables, "positivity_with_ci.csv"))
  print(positivity)
  ...
  if (isTRUE(cfg$statistics$fit_models) && power_note$inference_possible) {
    ...
    fit_pos <- try(fit_positivity_glmm(animal_tbl), silent = TRUE)
    if (!inherits(fit_pos, "try-error")) {
      readr::write_csv(tidy_model(fit_pos),
                       file.path(dirs$models, "positivity_glmm.csv"))
      saveRDS(fit_pos, file.path(dirs$models, "positivity_glmm.rds"))
    }

    ## Burden among positive cells, treated animals only: once the control is a
    ## floor, that is where the interesting variation lives.
    treated <- cell_data |>
      dplyr::filter(as.character(group) != cfg$design$control_group,
                    .data[[count_col]] >= threshold)

    if (nrow(treated) > 200 && dplyr::n_distinct(treated$region) > 1) {
      fit_burden <- try(fit_hurdle_nb(
        treated, count_col, fixed = "region",
        random = cfg$statistics$random_effects, zi = "~ 1"), silent = TRUE)
      if (!inherits(fit_burden, "try-error")) {
        readr::write_csv(tidy_model(fit_burden),
                         file.path(dirs$models, "burden_by_region_treated.csv"))
        icc <- report_icc(fit_burden)
        if (!is.null(icc)) capture.output(print(icc),
          file = file.path(dirs$models, "burden_icc.txt"))
      }
    }
    ...
    qc$pseudobulk_check <- pseudobulk_check(pseudobulk(cell_data, count_col))
  }

  ## --- 7. Figures ------------------------------------------------------------
  p1 <- plot_animal_points(
    animal_tbl, y_col = "pct_positive", x_col = "group", facet = "region",
    y_label = "% cells with detectable probe signal",
    title = "Compound distribution by tissue",
    subtitle = sprintf("Positivity threshold: >= %s puncta per cell (%s)",
                       threshold, cfg$readout$threshold_source),
    caption = paste0(
      "Detected compound is not active compound, and no limit of detection has ",
      "been published for\noligonucleotide ISH in any units. A low-signal ",
      "region means 'below an unknown threshold'."))
  save_plot(p1, "01_pct_positive_by_region.png", dirs$plots,
            width = cfg$output$figure_width %||% 9,
            height = cfg$output$figure_height %||% 6)

  p2 <- plot_threshold_sweep(qc$threshold_sweep)
  save_plot(p2, "02_threshold_sweep.png", dirs$plots, width = 10, height = 6)

  p3 <- plot_animal_points(
    animal_tbl, y_col = "median_among_pos", x_col = "group", facet = "region",
    y_label = "Median puncta per POSITIVE cell",
    title = "Burden among positive cells",
    subtitle = "Hurdle part 2. An index, not a copy number.",
    caption = paste0("Single-dot calibration: ",
                     cfg$readout$single_dot_calibration %||% "NOT RECORDED"))
  save_plot(p3, "03_burden_among_positive.png", dirs$plots,
            width = cfg$output$figure_width %||% 9,
            height = cfg$output$figure_height %||% 6)

  if (nrow(qc$staining)) {
    stain_cols <- setdiff(names(qc$staining),
                          c("group", "animal_id", "section_id"))
    if (length(stain_cols)) {
      joined <- dplyr::left_join(
        animal_tbl, qc$staining,
        by = intersect(c("group", "animal_id"), names(qc$staining)))
      p4 <- plot_staining_vs_readout(joined, stain_cols[1], "pct_positive")
      save_plot(p4, "04_staining_vs_readout.png", dirs$plots, width = 8, height = 6)
    }
  }
  ...
  write_run_manifest(cfg, dirs, manifest,
                     extra = list(pipeline = "01_biodistribution_chromogenic"))
  ...
  message("Read qc/ before the results, and EVIDENCE_BOUNDARIES.txt before ",
          "writing any interpretation.\n")

  invisible(list(cfg = cfg, dirs = dirs, cell_data = cell_data,
                 animal_tbl = animal_tbl, qc = qc))
}

## Entry point: run from the command line, or set CONFIG_PATH and source().
if (!interactive() && !exists("SOURCED_FOR_INTERACTIVE_USE")) {
  .args <- commandArgs(trailingOnly = TRUE)
  if (!exists("CONFIG_PATH")) {
    CONFIG_PATH <- if (length(.args)) .args[1] else
      stop("Usage: Rscript 01_biodistribution_chromogenic.R <config.yml>",
           call. = FALSE)
  }
